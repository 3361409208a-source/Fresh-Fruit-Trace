...
import type { Product, Batch, TraceEvent, TodayStats } from './types';
...
const store = {
  ...
  getEvents(): TraceEvent[] {
    return readJSON<TraceEvent[]>(EVENTS_FILE, []);
  },
  saveEvents(list: TraceEvent[]): void {
    writeJSON(EVENTS_FILE, list);
  },
  getEventsByBatch(batchId: string): TraceEvent[] {
    return this.getEvents().filter(e => e.batch_id === batchId).sort((a, b) => a.occurred_at - b.occurred_at);
  },
  addEvent(batchId: string, eventType: string, description: string): TraceEvent {
    const events = this.getEvents();
    const ev: TraceEvent = { id: Date.now() + Math.random(), batch_id: batchId, event_type: eventType, description, occurred_at: Math.floor(Date.now() / 1000) };
    events.push(ev);
    this.saveEvents(events);
    return ev;
  },
  deleteEventsByBatch(batchId: string): void {
