...
export default function BatchCreate() {
  ...
  const [form, setForm] = useState({ product_name: '', product_type_id: '', operator: '', weight: '', spec: '', notes: '' });
  ...
  const startCamera = async () => {
    try {
      ...
      setCameraError('无法访问摄像头：' + err.message + '。请允许浏览器访问摄像头权限。');
    }
  };
  ...
  const handleUploadVideo = async () => {
    ...
    } catch (err) { setError('视频上传失败：' + err.message); }
        ...
        {step === 3 && (
          <div style={{ textAlign: 'center', padding: '20px 0', animation: 'fadeIn 0.3s ease-out' }}>
            ...
            <div style={{ display: 'flex', gap: 12, justifyContent: 'center' }}>
              <button onClick={() => navigate(`/batches/${batchId}`)} style={btnPrimaryStyle}>
                <Apple size={18} /> 查看详情 & 打印标签
              </button>
              <button onClick={() => { setBatchId(null); setStep(0); setForm({ product_name: '', product_type_id: '', operator: '', weight: '', spec: '', notes: '' }); setRecordedBlob(null); setRecordingTime(0); setVideoUploaded(false); }} style={{ ...btnStyle, color: '#374151', background: '#f3f4f6', border: 'none' }}>
                继续新批次
              </button>
            </div>
          </div>
