...
export default function BatchList() {
  ...
  const [filters, setFilters] = useState({ product_name: '', status: '', date: '' });
            ...
            <tbody>
              {loading ? (
                ...
                batches.map(batch => {
                  ...
                  const expired = batch.expire_at && batch.expire_at < now;
...
const primaryBtnStyle = { display: 'flex', alignItems: 'center', gap: 8, background: 'linear-gradient(135deg, #16a34a, #22c55e)', color: 'white', border: 'none', borderRadius: 10, padding: '10px 18px', fontWeight: 600, fontSize: 14, cursor: 'pointer' };
const iconBtnStyle = { display: 'flex', alignItems: 'center', padding: '10px', border: '1.5px solid #e5e7eb', borderRadius: 10, cursor: 'pointer', background: 'white', color: '#6b7280' };
const tableActionBtn = (bg, color) => ({ background: bg, color, border: 'none', borderRadius: 8, padding: '6px 8px', cursor: 'pointer', display: 'flex', alignItems: 'center' });
